...
//  }
    ...
    int tryStep(sf::Vector2i from, sf::Vector2i to){
        ...
        switch (MASKF&moved) {
            case ROOK: if (to.x != from.x and to.y != from.y) return 0;
                break;
            case BISHOP: if (to.x + to.y != from.x + from.y and to.x - to.y != from.x - from.y) return 0;
                break;
            case QUEEN: if (to.x != from.x and to.y != from.y and to.x + to.y != from.x + from.y
                and to.x - to.y != from.x - from.y)
                return 0;
                break;
            case KING: if (abs(to.x-from.x)>1 or abs(to.y-from.y)>1) return 0;
                break;
            case PAWN: if //omg
        }
        figure taken = this->map[getField(from.x, from.y)];
        }
    };
